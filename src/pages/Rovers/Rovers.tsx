...
const Rovers = () => {
  ...
  const roversList = mockRovers as RoverItem[];
  const filteredRovers = useMemo(() => {
    return roversList.filter((rover) => {
      const matchesSearch =
    ...
    <div className="space-y-6 pt-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="rounded-xl bg-card p-4 border">
          ...
          <h2 className="text-2xl font-bold">{roversList.length}</h2>
        </div>
        <div className="rounded-xl bg-card p-4 border">
          <p className="text-sm text-muted-foreground">Active</p>
          <h2 className="text-2xl font-bold text-green-500">{roversList.filter((r) => r.status === "active").length}</h2>
        </div>
        <div className="rounded-xl bg-card p-4 border">
          <p className="text-sm text-muted-foreground">Idle</p>
          <h2 className="text-2xl font-bold text-yellow-500">{roversList.filter((r) => r.status === "idle").length}</h2>
        </div>
        <div className="rounded-xl bg-card p-4 border">
          <p className="text-sm text-muted-foreground">Problem</p>
          <h2 className="text-2xl font-bold text-red-500">{roversList.filter((r) => r.status === "problem").length}</h2>
        </div>
      </div>
      ...
      {selectedRover && (
        <RoverReportModal rover={selectedRover} onClose={() => setSelectedRover(null)} />
      )}
    </div>
  );
};
