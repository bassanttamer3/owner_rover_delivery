...
const Orders = () => {
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      case 'active':
        return (
          <Badge className="bg-success text-success-foreground animate-pulse/50">
            In Progress
          </Badge>
        );
      case 'problem':
        ...
        <Card className="p-4 flex-1 flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-2xl font-bold text-destructive mt-1">
              {orders.filter((o) => o.status === 'problem').length}
            </p>
          </div>
        </Card>
      </div>
                  ...
                  <TableCell>
                    <div className="flex items-center gap-2">
                      <div className="flex-1 bg-muted rounded-full h-2 max-w-[100px]">
                        <div
                          className="bg-gradient-to-r from-primary to-primary/70 h-2 rounded-full transition-all hover:h-3"
