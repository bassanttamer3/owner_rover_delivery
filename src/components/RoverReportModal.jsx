// RoverReportModal.jsx
import { Dialog, DialogContent } from "@/components/ui/dialog";
import { Badge } from "@/components/ui/badge";
import { Progress } from "@/components/ui/progress";

const RoverReportModal = ({ rover, onClose }) => {
        ...
        <div className="border rounded-xl p-5 bg-muted/40 mb-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-4">
            <div>
              ...
              <Progress value={rover.battery} />
              <p className="text-sm font-medium mt-1">{rover.battery}%</p>
            </div>

            <div>
              <p className="text-sm text-muted-foreground">Speed</p>
              <p className="font-semibold">{rover.speed} mph</p>
            </div>
        ...
        <div className="mb-6">
          ...
          {rover.currentOrder ? (
            <div className="border rounded-lg p-4 grid grid-cols-2 gap-4">
              <p><b>ID:</b> {rover.currentOrder.id}</p>
              <p><b>ETA:</b> {rover.currentOrder.eta}</p>
              <p><b>From:</b> {rover.currentOrder.from}</p>
              <p><b>To:</b> {rover.currentOrder.to}</p>
            </div>
          ) : (
